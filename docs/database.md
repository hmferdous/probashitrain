...
> **Migration status:** Applied. Use `as any` casts until `supabase gen types` is re-run to regenerate `types.ts`.
...
> **Migration status:** Applied. Branch assignments are currently stored in `localStorage` keyed by `user_branches_<centerId>` (demo mode) — the DB table exists but enforcement is not yet wired to the frontend query layer.
...
> **Migration status:** Applied. Invites are currently stored in `localStorage` keyed by `invites_<centerId>` (demo mode) — the DB table exists but the registration-via-token flow is not yet built.
